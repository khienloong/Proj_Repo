...
void doHelp()
{
	...
	printf("The -h or --help option will bring you to the help screen regardless of how many other options are set.\n\n");
	printf("Options List:\n");
	printf(" -h or --help: Takes you to help screen\n");
	printf(" -g or --get: Executes GET request with the URL given by user.\n");
	printf(" -o or --post: Exectues POST request with the URL and string given by user\n");
	printf(" -p or --put: Executes PUT request with URL and string given by user.\n");
	printf(" -d or --delete: Executes DELETE request with URL given by user\n");

	printf("\n");
	printf("Enter a URL with the -u or --url option:\n");
	printf("-u http://url.com or --url http://url.com\n");
	printf("\n");
	printf("The string MUST be the last value entered.  It must be one word without spaces; if it contains spaces, the entire string must be contained in double-quotes (\"This is an example of a string\")\n\n");

	printf("Below is an example of a proper CLI command:\n");
	printf("./<filename> -g -u <url>\n");
	printf("\n");
	printf("POST, PUT and DELETE need strings as the last option of the command. The command would look like:\n");
	printf("./<filename> -p -u <url> <string>\n\n");

	printf("NOTE: The string you provide may have a maximum of 20 characters. If it includes spaces, it must be wrapped in double-quotes (\").\n");

	printf("\n");

	printf("The following characters are not allowed, using them will abort the program: !@#$%^&*(){}[];'<>?~`\n");
}
